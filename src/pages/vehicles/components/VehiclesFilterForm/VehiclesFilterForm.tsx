...
export const VehiclesFilterForm = ({
  ...
  const { register, handleSubmit, reset } = useForm<VehiclesFormData>({
    defaultValues: defaultValues ?? { name: "", vehicle_class: "" },
  });

  const classes = useMemo(() => {
    const all = resourceList.map((v) => v.vehicle_class).filter(Boolean);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex gap-4 mb-4 flex-col md:flex-row items-end">
      <div className="w-full">
        ...
        <input {...register("name")} className="input input-bordered w-full" type="text" placeholder="Search vehicles..." />
      </div>

      <div className="w-full max-w-xs">
        <label className="label"><span className="label-text">Class</span></label>
        <select {...register("vehicle_class")} className="select select-bordered w-full">
